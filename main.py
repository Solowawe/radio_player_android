...
from kivymd.uix.list import OneLineAvatarIconListItem, IconLeftWidget, TwoLineListItem, ThreeLineListItem
...
class StationListItem(ThreeLineListItem):
    ...
    def __init__(self, track: Track, app_ref, **kwargs):
        ...
        self.on_long_press = lambda: self.app_ref._toggle_favorite(self.track)

    def _update_fav_text(self):
        """Обновить отображение звёздочки."""
        is_fav = self.app_ref.player.favorites.is_favorite(self.track.title)
        if is_fav:
            self.tertiary_text = '⭐  В избранном'
        else:
            self.tertiary_text = '☆  Нажмите долго, чтобы добавить в избранное'
...
class RadioPlayerApp(MDApp):
    ...
    def _rebuild_stations_list(self):
        ...
        fav_btn = self.screen.ids.fav_filter_btn
        if self._showing_favorites_only:
